...
(function () {
    ...
    deleteStudent = function (data, id) {
        return $.ajax({
            url: resourceUrl + '/' + id,
            type: 'POST',
            data:{_method: 'delete'},
            success: function (data) {
                $successMessage
                  .html('Student No ' + id + ' successfully deleted')
                  .show()
                  .fadeOut(2000);
                reloadStudents();
            },
    ...
    $(function () {
        ...
        $('#students-container').on('click', function (evt) {
            var id = Number($(event.target).parent().find('.student-id').html());
            console.log(id);
            var data = { id: id }
