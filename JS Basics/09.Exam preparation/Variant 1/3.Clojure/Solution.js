var testVar0 = ['(  def func    10)', '(def newFunc (+    func  2))', '(def sumFunc (+ func func newFunc 0 0 0))', '(* sumFunc 2)'];
var testVar1 = ['(def func (+ 5 2))', '(def func2 (/  func 5 2 1 0))', '(def func3 (/ func 2))', '(+ func3 func)'];
var testVar2 = [
    '(def joros 30)',
    '(def newFunc (-  101 10))',
    '(def tryFunc 500)',
    ...
    function calculate(expression) {
        ...
        switch (expression[0]) {
            ...
            case '/':
                result = evaluate(expression[1]);
                for (var i = 2; i < expression.length; i++) {
                    result = parseInt(result / evaluate(expression[i]));
                }
                if (Number.isNaN(result)) {
                    return result;
                }
